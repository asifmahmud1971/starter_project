      ...
      response.fold(
        ...
        (data) async {
          state.painAssessmentModel?.allPainAssessment
              ?.insert(0, data.savedData ?? AllPainAssessment());

          emit(state.copyWith(
              appStatus: AppStatus.success,
              painAssessmentModel: state.painAssessmentModel));
        },
      );
      GetContext.back();

      showCustomSnackBar(
          message: AppStrings.painAssessmentSubmittedSuccessfully.tr());
    ...
    try {
      ...
      response.fold(
        (failure) {
          ...
          showCustomSnackBar(
              isError: true, message: AppStrings.failedToUpdateMedication.tr());
        },
        (data) {
          final doseList =
              state.medicationModel?.painMedications?[medicationIndex].chart;

          if (doseList != null) {
            final doseIndex =
                doseList.indexWhere((e) => e.id == int.parse(id.toString()));

            if (doseIndex != -1) {
              doseList[doseIndex].taken = "Yes";
            } else {
              showCustomSnackBar(
                  isError: true, message: AppStrings.doesNotFoundInChart.tr());
            }

            emit(state.copyWith(
                appStatus: AppStatus.success,
                medicationModel: state.medicationModel));
          } else {
            showCustomSnackBar(
                isError: true, message: AppStrings.noMedicationAvailable.tr());
            emit(state.copyWith(appStatus: AppStatus.failure));
          }
        },
      );
      ...
      print(stackTrace);
      showCustomSnackBar(
          isError: true, message: AppStrings.anUnExpectedErrorOccur.tr());
      emit(state.copyWith(appStatus: AppStatus.failure));
    } finally {
      dismissProgressDialog();
    }
