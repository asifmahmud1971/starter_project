...
class TaskCubit extends Cubit<TaskState> {
  ...
  Future<void> getTaskData() async {
    showProgressDialog();
    emit(state.copyWith(appStatus: AppStatus.loading, taskModel: TaskModel()));
    try {
    ...
    try {
      ...
      response.fold(
        ...
        (data) async {
          ...
          final index = state.taskModel?.tasks
              ?.indexWhere((e) => e.taskId == data.data?.taskId);

          state.taskModel?.tasks?[index ?? 0] = data.data ?? StaffTasks();
          emit(state.copyWith(
              appStatus: AppStatus.success, taskModel: state.taskModel));
        },
      );
